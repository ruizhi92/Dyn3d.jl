...
IMPLICIT NONE
    ...
    DO i = 1, input_body%nbody
        ...
        IF(ALLOCATED(body_system(i)%verts)) THEN
            ...
            DO j = 1, SIZE(body_system(i)%verts,1)
                WRITE(2017,'(3F20.15)') body_system(i)%verts(j,:)
            END DO
        END IF
        IF(ALLOCATED(body_system(i)%verts_i)) THEN
            WRITE(2017,'(A)') 'verts_i:'
            DO j = 1, SIZE(body_system(i)%verts_i,1)
                WRITE(2017,'(3F20.15)') body_system(i)%verts_i(j,:)
            END DO
        END IF
        WRITE(2017,'(A,3F20.15)') 'x_c:',body_system(i)%x_c(:)
        WRITE(2017,'(A,3F20.15)') 'x_0:',body_system(i)%x_0(:)
        WRITE(2017,'(A,F20.15)') 'mass:',body_system(i)%mass
        WRITE(2017,'(A)') 'inertia_c:'
        DO j = 1, SIZE(body_system(i)%inertia_c,1)
            WRITE(2017,'(6F20.15)') body_system(i)%inertia_c(j,:)
        END DO
        WRITE(2017,'(A)') 'inertia_j:'
        DO j = 1, SIZE(body_system(i)%inertia_j,1)
            WRITE(2017,'(6F20.15)') body_system(i)%inertia_j(j,:)
        END DO
        WRITE(2017,'(A)') 'Xj_to_c:'
        DO j = 1, SIZE(body_system(i)%Xj_to_c,1)
            WRITE(2017,'(6F20.15)') body_system(i)%Xj_to_c(j,:)
        END DO
        WRITE(2017,'(A)') 'Xb_to_i:'
        DO j = 1, SIZE(body_system(i)%Xb_to_i,1)
            WRITE(2017,'(6F20.15)') body_system(i)%Xb_to_i(j,:)
        END DO
        WRITE(2017,'(A)') 'Xp_to_b:'
        DO j = 1, SIZE(body_system(i)%Xp_to_b,1)
            WRITE(2017,'(6F20.15)') body_system(i)%Xp_to_b(j,:)
        END DO

        WRITE(2017,'(/)')
        WRITE(2017,'(A,6F20.15)') 'v:', body_system(i)%v(:,1)
        WRITE(2017,'(A,6F20.15)') 'c:', body_system(i)%c(:,1)
        WRITE(2017,'(A,6F20.15)') 'pA:', body_system(i)%pA(:,1)

        WRITE(2017,'(A)') 'Ib_A:'
        DO j = 1, SIZE(body_system(i)%Ib_A,1)
            WRITE(2017,'(6F20.15)') body_system(i)%Ib_A(j,:)
        END DO
    ...
    DO i = 1, input_body%nbody
        ...
        WRITE(2017,'(A,6F20.15)') 'shape1:', joint_system(i)%shape1(:)
        WRITE(2017,'(A,6F20.15)') 'shape2:', joint_system(i)%shape2(:)
        ...
        DO j = 1, joint_system(i)%nudof
            ...
            IF(joint_system(i)%joint_dof(j)%dof_type == 'passive') THEN
                WRITE(2017,'(A,F20.15)') 'stiff: ',joint_system(i)%joint_dof(j)%stiff
                WRITE(2017,'(A,F20.15)') 'damp: ',joint_system(i)%joint_dof(j)%damp
            ELSE IF(joint_system(i)%joint_dof(j)%dof_type == 'active') THEN
                ...
                IF(ALLOCATED(joint_system(i)%joint_dof(j)%motion_params)) THEN
                    ...
                    WRITE(2017,'(F20.15)',ADVANCE="NO") joint_system(i)%joint_dof(j)%motion_params(k)
                    END DO
                END IF
                WRITE(2017,'(/)')
            END IF
        ...
        WRITE(2017,'(A,6F20.15)') 'vJ:', joint_system(i)%vJ(:,1)
        WRITE(2017,'(A,6F20.15)') 'cJ:', joint_system(i)%cJ(:,1)
    ...
    WRITE(2017,'(A,3F20.15)') 'params%gravity:', system%params%gravity
    WRITE(2017,'(A,F20.15)') 'params%dt:', system%params%dt
    WRITE(2017,'(A,F20.15)') 'params%tf:', system%params%tf
    ...
    IF(ALLOCATED(system%soln%t)) THEN
        DO i = 1, SIZE(system%soln%t)
            WRITE(2017,'(F20.15)',ADVANCE="NO") system%soln%t(i)
            DO j = 1, SIZE(system%soln%y,2)
                WRITE(2017,'(F20.15)',ADVANCE="NO") system%soln%y(i,j)
            END DO
            WRITE(2017,'(/)')
        END DO
    END IF
    ...
    IF(ALLOCATED(system%kindata)) THEN
        DO i = 1, SIZE(system%kindata,1)
            DO j = 1, SIZE(system%kindata,2)
                WRITE(2017,'(F20.15)',ADVANCE="NO") system%kindata(i,j)
            END DO
            WRITE(2017,'(/)')
        END DO
    END IF
