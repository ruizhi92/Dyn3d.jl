...
!                 next by revolute joint. All joints are passive, the body
!                 system is under gravity.
!
!  Input        :
!
!  Input/output :
...
IMPLICIT NONE
    ...
    !------------------------------------------------------------------------
    REAL(dp)                        :: tf
    INTEGER                         :: nbody,i,njoint,nstep,scheme,ndim
    ...
    input_joint(1)%shape1(1:3) = (/ 0.0_dp, 0.0_dp, 0.0_dp /)
    ...
    input_joint(1)%joint_dof(1) = default_dof_passive
    input_joint(1)%joint_dof(1)%stiff = 0.0_dp
    input_joint(1)%joint_dof(1)%damp = damp
    !-------------- Other joints --------------
...
END SUBROUTINE config_2d_fallobj