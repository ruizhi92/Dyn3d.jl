...
IMPLICIT NONE
    ...
    REAL(dp),DIMENSION(6,6)                         :: Xtemp,Xtemp2,Xtemp_inv
    ...
    ! for body 1
    CALL trans_matrix(body_system(1)%q(4:6,1),body_system(1)%q(1:3,1), Xtemp2)
    body_system(1)%Xp_to_b = MATMUL(Xtemp2,body_system(1)%Xp_to_b)

    ! for body 2 to n
    DO i = 2, system%nbody
        p_id = body_system(i)%parent_id
        CALL trans_matrix(body_system(p_id)%q(4:6,1),body_system(p_id)%q(1:3,1), Xtemp)
        CALL inverse(Xtemp,Xtemp_inv)
        CALL trans_matrix(body_system(i)%q(4:6,1),body_system(i)%q(1:3,1), Xtemp2)
        body_system(i)%Xp_to_b = MATMUL(Xtemp2,MATMUL(body_system(i)%Xp_to_b,Xtemp_inv))
        CALL write_matrix(Xtemp)
        WRITE(*,*) '----------------------------'
        CALL write_matrix(body_system(i)%Xp_to_b)
        WRITE(*,*) '----------------------------'
        CALL write_matrix(Xtemp2)
        !body_system(i)%Xp_to_b = MATMUL(Xtemp2,body_system(i)%Xp_to_b)
    END DO
